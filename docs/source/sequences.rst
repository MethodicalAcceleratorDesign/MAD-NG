...
**siter**
   ...
   between the sequence attribute ``minlen`` and the ``minlen`` from the :doc:`constant <mathfunc>` module.
...
**install**
   ...
   the :doc:`utility <utilfunc>` module for details. The *implicit* drifts are checked after each element installation.
