...
**minlen**
   A *number* specifying the minimal length ``[m]`` to generate *implicit* drifts between elements in :math:`s`-iterators generated by the method ``:siter``. This attribute is automatically set to :math:`10^{-6}` m when a sequence is created within the MADX environment. (default: ``nil``)
...
**siter**
   ...
   between the sequence attribute ``minlen`` and the ``minlen`` from the :doc:`constant <mathfuncs>` module.
...
**install**
   ...
   the :doc:`miscellaneous <miscellaneous>` module for details. The *implicit* drifts are checked after each element installation.
