...
{ CHKX; ensure(n>0, "invalid vector/matrix size");
  FOR(i,n) { if (x[i] != v) return FALSE; } return TRUE; }

void mad_vec_fill (num_t x, num_t r[], ssz_t n)
{ CHKR; ensure(n>0, "invalid vector/matrix size");
  FOR(i,n) r[i] = x; }

void mad_vec_copy (const num_t x[], num_t r[], ssz_t n)
{ CHKXR; ensure(n>0, "invalid vector/matrix size");
  if (x == r) return;
  uintptr_t x_ = (uintptr_t)x, r_ = (uintptr_t)r;
  if (x_ > r_) FOR(i,n) r[    i] = x[    i];
  else         FOR(i,n) r[n-1-i] = x[n-1-i]; }
...
{ assert(r && (re || im)); ensure(n>0, "invalid vector/matrix size");
...
{ CHKX; ensure(n>0, "invalid vector/matrix size");
  num_t r=0; FOR(i,n) r += x[i]; return r; }
...
{ CHKXY; ensure(n>0, "invalid vector/matrix size");
  num_t r=0;
  FOR(i,n) r += creal(conj(x[i]-y[i])*(x[i]-y[i]));
  return sqrt(r);
}

void mad_vec_add (const num_t x[], const num_t y[], num_t r[], ssz_t n)
{ CHKXYR; ensure(n>0, "invalid vector/matrix size");
  FOR(i,n) r[i] = x[i] + y[i]; }
...
{ CHKYR; ensure(n>0, "invalid vector/matrix size");
  FOR(i,n) r[i] = x - y[i]; }
...
{ assert(a && x && r); ensure(n>0, "invalid vector/matrix size");
  if (k == 0) return;
  int j = k%8;
...
{ assert(x && (re || ri)); ensure(n>0, "invalid vector/matrix size");
...
{ CHKX; ensure(n>0, "invalid vector/matrix size");
  cpx_t r=0; FOR(i,n) r += x[i]; return r; }
