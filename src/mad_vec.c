...
{ assert( r && (x || y) );
  if (x && y) for (ssz_t i=0; i < n; i++) r[i] = CNUM(x[i],y[i]);
  else if (x) for (ssz_t i=0; i < n; i++) r[i] =      x[i]      ;
  else        for (ssz_t i=0; i < n; i++) r[i] = CNUM(0   ,y[i]);
}

num_t mad_vec_dot (const num_t x[], const num_t y[], ssz_t n)
{ CHKXY; num_t r=0; for (ssz_t i=0; i < n; i++) r += x[i] * y[i]; return r; }

cnum_t mad_vec_dotv (const num_t x[], const cnum_t y[], ssz_t n)
{ CHKXY; cnum_t r=0; for (ssz_t i=0; i < n; i++) r += x[i] * y[i]; return r; }
...
{ CHKYR; for (ssz_t i=0; i < n; i++) r[i] = x - y[i]; }
...
{ CHKXR; for (ssz_t i=0; i < n; i++) r[i] = conj(x[i]); }

cnum_t mad_cvec_dot (const cnum_t x[], const cnum_t y[], ssz_t n)
{ CHKXY; cnum_t r=0; for (ssz_t i=0; i < n; i++) r += conj(x[i])*y[i]; return r; }
...
{ CHKXY; num_t r=0;
  for (ssz_t i=0; i < n; i++) r += SQR(creal(x[i]) - y[i]) + SQR(cimag(x[i]));
  return sqrt(r);
}
...
mad_cvec_nfft (const cnum_t x[], const num_t x_node[], cnum_t r[], ssz_t n, ssz_t nr)
{
  ...
  if (x_node || precomp) {
    for (ssz_t i=0; i < n; i++)  // adjoint transform needs -x_node
...
mad_cvec_infft (const cnum_t x[], const num_t r_node[], cnum_t r[], ssz_t n, ssz_t nx)
{
  ...
  if (r_node || precomp) {
    for (ssz_t i=0; i < n; i++) // forward transform needs -r_node
