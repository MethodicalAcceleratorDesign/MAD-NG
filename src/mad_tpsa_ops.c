...
FUN(scl) (const T *a, NUM v, T *c)
{
  ...
  if (!v || a->hi == 0) {
    FUN(set0)(c,0,v*a->coef[0]); DBGFUN(<-); return;
  }
...
FUN(powi) (const T *a, int n, T *c)
{
  ...
  switch (n) {
    case 0: FUN(set0) (c, 0, 1); break; // ok: no copy
...
FUN(derivm) (const T *a, T *r, ssz_t n, const ord_t mono[n])
{
  ...
  // ord 0 & setup
  FUN(set0)(c, 0, FUN(geti)(a,idx) * der_coef(idx,idx,der_ord,d));
...
{            //    a           x       b           y      c      r
  ...
  c->nz = mad_bit_hcut(a->nz|b->nz, c_hi);
  if (!c->nz) {
    FUN(set0)(c, 0, c1*a->coef[0]+c2*b->coef[0]+c3); DBGFUN(<-); return;
  }
