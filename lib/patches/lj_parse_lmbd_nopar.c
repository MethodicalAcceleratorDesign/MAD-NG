...
static BCReg parse_params(LexState *ls, int needself, int islambda)
{
  FuncState *fs = ls->fs;
  BCReg nparams = 0;
  int has_param = islambda && ls->c != ' ' && ls->c != '\t';    /* LD: 2016.04.07 */

  if (islambda) lj_lex_next(ls); else lex_check(ls, '(');       /* LD: 2016.04.07 */
  if (needself) var_new_lit(ls, nparams++, "self");
  if (has_param || (!islambda && ls->tok != ')')) {             /* LD: 2016.04.07 */
    do {
      if (ls->tok == TK_name || (!LJ_52 && ls->tok == TK_goto)) {
  ...
  if (!islambda) lex_check(ls, ')');                            /* LD: 2016.04.07 */
  return nparams;
}
...
static void parse_body(LexState *ls, ExpDesc *e, int needself, int islambda, BCLine line)
{
  ...
  fs.linedefined = line;
  fs.numparams = (uint8_t)parse_params(ls, needself, islambda); /* LD: 2016.04.07 */
  fs.bcbase = pfs->bcbase + pfs->pc;
  fs.bclim = pfs->bclim - pfs->pc;
  bcemit_AD(&fs, BC_FUNCF, 0, 0);  /* Placeholder. */
  if (islambda) parse_return(ls, islambda);                     /* LD: 2016.04.07 */
  else {
    parse_chunk(ls);
  ...
  if (!islambda) lj_lex_next(ls);                               /* LD: 2016.04.07 */
}
...
static void parse_return(LexState *ls, int islambda)
{
  BCIns ins;
  FuncState *fs = ls->fs;
  int has_list = islambda && lex_opt(ls, '(');                  /* LD: 2016.04.07 */

  if (!islambda) lj_lex_next(ls); /* Skip 'return'. */          /* LD: 2016.04.07 */
  fs->flags |= PROTO_HAS_RETURN;
  if (parse_isend(ls->tok) || ls->tok == ';') {  /* Bare return. */
    ins = BCINS_AD(BC_RET0, 0, 1);
  } else {  /* Return with one or more values. */
    ExpDesc e;  /* Receives the _last_ expression in the list. */
    BCReg nret = !islambda || has_list ? expr_list(ls,&e) : (expr(ls,&e),1); /* LD: 2016.04.07 */
  ...
  if (islambda && has_list) lex_check(ls, ')');                 /* LD: 2016.04.07 */
